...
void ILI9341_SPI_Send_Image(uint8_t* image_array[])
{
	...
	for(uint32_t byte_order = 0; byte_order < sizeof(image_array); byte_order = byte_order + 32768)
	{
		if((sizeof(image_array) - byte_order) < 32768)
		{
			HAL_SPI_Transmit(&lcd_spi, &image_array[byte_order], sizeof(image_array) - byte_order, 100);
		}
		else
		{
			HAL_SPI_Transmit(&lcd_spi, &image_array[byte_order], 32768, 100);
		}
